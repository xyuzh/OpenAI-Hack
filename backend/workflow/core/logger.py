...
def get_file_handler(
    ...
    file_name = f'usebase_{timestamp}.log'
...
usebase_logger = logging.getLogger('usebase')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
usebase_logger.setLevel(current_log_level)

if DEBUG:
    usebase_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    LOG_TO_FILE = True
    usebase_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    usebase_logger.addHandler(json_log_handler(current_log_level))
else:
    usebase_logger.addHandler(get_console_handler(current_log_level))

usebase_logger.addFilter(SensitiveDataFilter(usebase_logger.name))
usebase_logger.propagate = False
usebase_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of usebase/core (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    usebase_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    usebase_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        usebase_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class usebaseLoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(
        self, logger: logging.Logger = usebase_logger, extra: dict | None = None
    ) -> None:
        self.logger = logger
        self.extra = extra or {}
